...
const ViewProvider: React.FC = () => {
  ...
  console.log("filteredProviders", currentItems);
  const handleApply = () => {
      if(paid && unpaid){
        handleViewDocument()
      }else if(paid){
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex flex-col md:flex-row lg:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  {/* Filter Button */}
                  <button className="flex items-center justify-center px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors" onClick={() => setShowFilter(!showFilter)}>
                  ...
                  {showFilter && (
                    <div className="w-full max-w-sm mx-auto p-4 bg-white shadow rounded-2xl absolute right-6 top-16 z-20 cursor-pointer">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <h2 className='text-sm font-thin mb-3 cursor-pointer bg-slate-100 hover:bg-slate-200 hover:font-semibold p-1 px-2 rounded-lg hover:scale-105 transition-transform duration-200 ease-in-out' onClick={() => setShowFilter(false)}>Close</h2>
                      </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {currentItems.map((provider, index) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${provider.status === 'pending'
                                  ...
                                  <div className="flex flex-col space-y-6">
                                    ...
                                    <div className='grid grid-cols-2 gap-4 px-4'>
                                      <button className='px-4 py-3 bg-green-100 text-green-700 font-semibold rounded-lg hover:bg-green-600 hover:text-white transition-all duration-200' onClick={() => {
                                        handleEditClick(provider._id.toString())
                                        setIsEditOpen(false)
